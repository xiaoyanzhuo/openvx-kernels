...
int main(int argc, char *argv[])
{
	...
	// step 2: Images (in/output)
		...
		vx_scalar scalar_alpha = vxCreateScalar(context, VX_TYPE_FLOAT32, &alpha);
	...
	// step 4: Graph nodes construstion	
		...
		{
			vx_node nodes[] = {
				vxAccumulateWeightedImageNode(graph, images[0], scalar_alpha, images[1]),
			};
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				status = vxProcessGraph(graph);
			// WRITE RESULT
				vxuFWriteImage(context, images[1], "../raw/accu_weight_640x480.bw"); 
//				vxuFWriteImage(context, images[1], "./accu_weight_640x480.pgm"); 
