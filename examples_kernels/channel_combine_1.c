...
int main(int argc, char *argv[])
{
	...
	// step 2: Images (in/output)
		vx_image images[] = {
			...
			vxCreateImage(context, width, height, VX_DF_IMAGE_RGB),    /* 4: rgb */
		};
		...
		vxuFReadImage(context, "../raw/colorbars_640x480_I444.rgb", images[4]);
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				...
				if (status == VX_SUCCESS)
				{
					// make sure images[3] == images[4]
					status = vxuCompareImages(context, images[3], images[4], &diffs);
