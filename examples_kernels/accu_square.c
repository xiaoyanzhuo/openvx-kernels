...
int main(int argc, char *argv[])
{
	...
	// step 2: Images (in/output)
		...
		vx_uint32 shift = 4;
		vx_scalar scalar_shift = vxCreateScalar(context, VX_TYPE_UINT32, &shift);
	...
	// step 4: Graph nodes construstion	
		...
		{
			vx_node nodes[] = {
				vxAccumulateSquareImageNode(graph, images[0], scalar_shift, images[1]),
			};
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				status = vxProcessGraph(graph);
			// WRITE RESULT
				vxuFWriteImage(context, images[1], "../raw/accu_square_640x480.bw"); 
