...
int main(int argc, char *argv[])
{
	...
	// step 2: Images (in/output)
		vx_image images[] = {
			...
			vxCreateImage(context, width, height, VX_DF_IMAGE_S16),  /* 1: grad_x */
			vxCreateImage(context, width, height, VX_DF_IMAGE_S16),  /* 2: grad_y */
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				status = vxProcessGraph(graph);
			// WRITE RESULT
				vxuFWriteImage(context, images[1], "../raw/sobel_gradh_640x480.bw");
				vxuFWriteImage(context, images[2], "../raw/sobel_gradv_640x480.bw"); 
