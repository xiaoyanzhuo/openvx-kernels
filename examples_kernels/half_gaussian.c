...
int main(int argc, char *argv[])
{
	...
	vx_uint32 i;
	vx_uint32 w2 = (width + 1)/2, h2 = (height + 1)/2;
	vx_int32 kernel_size = 3;  //1, 3, 5
	...
	// step 4: Graph nodes construstion	
		...
		{
			vx_node nodes[] = {
				vxHalfScaleGaussianNode(graph, images[0], images[1], kernel_size),
			};
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				status = vxProcessGraph(graph);
			// WRITE RESULT
				vxuFWriteImage(context, images[1], "../raw/half_gaussian.pgm"); 
