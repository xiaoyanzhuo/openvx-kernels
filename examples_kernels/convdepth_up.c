...
int main(int argc, char *argv[])
{
	...
	// step 2: Images (in/output)
		...
		vxuFReadImage(context, "../raw/absdiff_640x480.pgm", images[0]);
	...
	// step 4: Graph nodes construstion	
		...
		{
			vx_node nodes[] = {
				vxConvertDepthNode(graph, images[0], images[1], policy, sshift),
			};
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				status = vxProcessGraph(graph);
			// WRITE RESULT
				vxuFWriteImage(context, images[1], "../raw/convdepth_up_640x480.bw"); 
