...
int main(int argc, char *argv[])
{
	...
	// step 3: Images (in/output)
		vx_image images[] = {
			...
			vxCreateImage(context, width, height, VX_DF_IMAGE_U8),   /* 3: phase */
	...
	// step 4: Graph nodes construstion	
		if (graph)
		{
			vx_node nodes[] = {
				...
				vxPhaseNode(graph, images[1], images[2], images[3]),
			};
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				status = vxProcessGraph(graph);
			// WRITE RESULT
				vxuFWriteImage(context, images[3], "../raw/phase_640x480.pgm");
