...
{
	vx_status status = VX_SUCCESS;
	vx_uint32 width = img_w1;
	vx_uint32 height = img_h1;
	vx_uint32 w2 = img_w2, h2 = img_h2;  //scaled image size
	vx_uint32 i;
	...
	// step 2: Images (in/output)
		vx_image images[] = {
			...
			vxCreateImage(context, w2, h2, VX_DF_IMAGE_U8),
		};
		vxuFReadImage(context, "lena_512x512.pgm", images[0]);
	...
	// step 6: GRAPH EXECUTION
			if (status == VX_SUCCESS)
			{
				status = vxProcessGraph(graph);
			// WRITE RESULT
				vxuFWriteImage(context, images[1], "lena_4096x4096.pgm");
