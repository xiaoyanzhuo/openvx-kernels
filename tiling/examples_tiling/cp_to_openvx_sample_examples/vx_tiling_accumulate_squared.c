...
#include <math.h>
/*! \file
 * \example vx_tiling_accumulate_squared.c
 */
 ...
 */
//! [accumulate_squared_tiling_function]
void accumulate_squared_image_tiling(void * VX_RESTRICT parameters[VX_RESTRICT],
                        ...
                        vx_size tile_memory_size)
{
    ...
    vx_uint32 *shift = (vx_uint32 *)parameters[1];
        ...
        for (i = 0u; i < vxTileWidth(in, 0); i+=vxTileBlockWidth(in))
        {
            for (n = 0u; n < vxTileBlockHeight(in); n++)
            {
                for (m = 0u; m < vxTileBlockWidth(in); m++)
                {
                    ...
                    vx_int32 res = ((vx_int32)pixel * (vx_int32)pixel);
                    res = ((vx_uint32)pixel_out) + (res >> (*shift));
                    if (res > INT16_MAX)
                        res = INT16_MAX;
