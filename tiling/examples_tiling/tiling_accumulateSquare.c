...
vx_node vxTilingAccumulateSquaredNode(vx_graph graph, vx_image in, vx_scalar shift, vx_image out)
{
    vx_reference params[] = {
        (vx_reference)in,
        (vx_reference)shift,
...
int main(int argc, char *argv[])
{
    ...
    if (vxGetStatus((vx_reference)context) == VX_SUCCESS)
    {
        ...
        vx_image images[] = {
                ...
                vxCreateImage(context, img_w, img_h, VX_DF_IMAGE_S16), // 2:accu_squared
        };
        vx_uint32 a = 4;
        vx_scalar shift = vxCreateScalar(context, VX_TYPE_UINT32, &a);
        ...
        if (status == VX_SUCCESS)
        {
            ...
            if (vxGetStatus((vx_reference)graph) == VX_SUCCESS)
            {
                vx_node nodes[] = {
                    ...
                    vxTilingAccumulateSquaredNode(graph, images[1], shift, images[2]),
                    vxFWriteImageNode(graph, images[2], "tiling_accumulateSquared_lena_512x512.pgm"),
                };
